 ...
 */
export function groupRecordsByIdentity(records: NormalizedRecord[]): NormalizedRecord[][] {
 ...
 */
export function mergeCandidateGroup(group: NormalizedRecord[]): CanonicalProfile {
  const merged: Partial<CanonicalProfile> = {
  ...
  const pickWinner = (field: keyof CanonicalProfile) => {
  ...
  const fieldsToMerge: (keyof CanonicalProfile)[] = [
  ...
  return merged as CanonicalProfile;
}
